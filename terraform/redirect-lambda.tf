...
# The CloudWatch log group that the Lambda function will write logs to
resource "aws_cloudwatch_log_group" "redirect-lambda-log-group" {
  name              = "/aws/lambda/${aws_lambda_function.redirect-lambda-function.function_name}"
  retention_in_days = 1

  tags = {
    Name = "redirect-example-lambda-log-group"
  }
}